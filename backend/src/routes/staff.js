...
router.get("/:id", async (req, res, next) => {
  try {
    const staffMember = await getEmployeeById(req.params.id);
...
router.post("/", async (req, res, next) => {
  try {
    const staffMember = await createEmployee(req.body || {}, req.user && req.user.sub);
...
router.put("/:id", async (req, res, next) => {
  try {
    const staffMember = await updateEmployee(req.params.id, req.body || {}, req.user && req.user.sub);
    res.json({ data: staffMember });
  } catch (error) {
    next(error);
  }
